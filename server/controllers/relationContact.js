...
const updateContact = (req, res) => {
  ...
  RelationContact.findOne({ _id, isDelete: false }, (err, leadContact) => {
    ...
    if (!leadContact) {
      ...
      leadContact.companyEmail = companyEmail
        ? companyEmail
        : leadContact.companyEmail;
      leadContact.companyPhone = companyPhone
        ? companyPhone
        : leadContact.companyPhone;

      leadContact.tags = tags ? tags : leadContact.tags;
      leadContact.socialLinks = socialLinks
        ? socialLinks
        : leadContact.socialLinks;
      leadContact.companyAddress = companyAddress
        ? companyAddress
        : leadContact.companyAddress;
...
const removeOther = async (req, res) => {
  try {
    ...
    const newOther = client.others.filter(
      (r) => String(r._id) != String(otherId)
    );
